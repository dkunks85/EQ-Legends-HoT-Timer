...
public sealed class MainForm : Form
{
    ...
    private readonly Button _watch = new()
    {
        Text = "Start Watching",
        AutoSize = true
    };
    ...
    private readonly CheckBox _learningMode = new()
    {
        Text = "Learning Mode",
        AutoSize = true,
    ...
    public MainForm()
    {
        ...
        _spells = new BindingList<SpellDefinition>(
            _store.LoadSpells());
            ...
            CharacterName)
        {
            ...
            LearningRequested = PromptLearning
        };

        _engine.Activity += message =>
            Ui(() => Log(message));

        _engine.TimersChanged += () =>
            Ui(RenderTimers);

        _engine.SpellDefinitionsChanged += () =>
            Ui(() =>
            {
                _grid.Refresh();
                SaveAll();
        ...
        _tailer.LineReceived += line =>
            _engine.Process(line);

        _tailer.Error += ex =>
            Ui(() => Log("Watcher error: " + ex.Message));

        BuildUi();
        BindGrid();
        StyleSpellGrid();

        _learningMode.CheckedChanged += (_, _) =>
            _engine.LearningEnabled = _learningMode.Checked;

        _hotPanel.ClientSizeChanged += (_, _) =>
            ResizeTimerCards();

        _buffPanel.ClientSizeChanged += (_, _) =>
            ResizeTimerCards();
    ...
    private TabPage TimersTab()
    {
        ...
        _timerSplit.Panel1.Controls.Add(
            Section("Healing-over-time", DockStyle.Top));

        _timerSplit.Panel2.Controls.Add(_buffPanel);
        _timerSplit.Panel2.Controls.Add(
            Section("Buffs", DockStyle.Top));
    ...
    private TabPage SetupTab()
    {
        ...
        defaults.Click += (_, _) =>
        {
            ...
            foreach (var spell in ConfigStore.Defaults())
                _spells.Add(spell);
        ...
        tools.Controls.Add(_learningMode);
    ...
    private void BindGrid()
    {
        ...
        AddCheck(
            nameof(SpellDefinition.Enabled),
            "Enabled",
            60);

        AddText(
            nameof(SpellDefinition.Name),
            "Spell",
            150);

        AddCombo(
            nameof(SpellDefinition.Category),
            "Category",
            80,
            ["HoT", "Buff"]);

        AddText(
            nameof(SpellDefinition.Duration),
            "Duration",
            75);

        AddCombo(
            nameof(SpellDefinition.DetectionMode),
            "Detection",
            135,
            ["Auto HoT Family", "Landing Message"]);

        AddText(
            nameof(SpellDefinition.MatchName),
            "Match name",
            130);

        AddText(
            nameof(SpellDefinition.LandingPattern),
            "Landing pattern",
            250);

        AddText(
            nameof(SpellDefinition.FadePattern),
            "Fade pattern",
            220);
        ...
        _grid.CellEndEdit += (_, e) =>
        {
            if (_grid.Columns[e.ColumnIndex].DataPropertyName ==
                    nameof(SpellDefinition.Name) &&
                _grid.Rows[e.RowIndex].DataBoundItem is
                    SpellDefinition spell &&
                string.IsNullOrWhiteSpace(spell.MatchName))
            {
                spell.MatchName = SpellNames.Base(spell.Name);
            }
        };
    }
    ...
    private LearningDecision PromptLearning(
        LearningCandidate candidate)
    {
        if (InvokeRequired)
        {
            return (LearningDecision)Invoke(
                new Func<LearningDecision>(
                    () => PromptLearning(candidate)));
        }
        ...
        buttons.Controls.Add(learn);
        layout.Controls.Add(buttons, 0, 3);

        dialog.Controls.Add(layout);
        dialog.AcceptButton = learn;
    ...
    private async Task ToggleWatchAsync()
    {
        ...
        if (!File.Exists(path))
        {
            MessageBox.Show(
                this,
                "Choose a valid EverQuest log file first.",
                "Log file",
    ...
    private void Browse(object? sender, EventArgs e)
    {
        ...
        {
            Filter =
                "EverQuest logs (eqlog_*.txt)|eqlog_*.txt|" +
                "Text files (*.txt)|*.txt|All files (*.*)|*.*",
            FileName = Path.GetFileName(_logPath.Text),
            InitialDirectory =
                Directory.Exists(Path.GetDirectoryName(_logPath.Text))
    ...
    private void RenderTimers()
    {
        ...
        foreach (var timer in _engine.Timers.OrderBy(t => t.End))
        {
            ...
            {
                _hotPanel.Controls.Add(HotCard(timer));
            }
            else
            {
                _buffPanel.Controls.Add(BuffRow(timer));
            }
        }
    ...
    private Control HotCard(ActiveTimer timer)
    {
        var panel = new Panel
        {
            Width = TimerCardWidth(_hotPanel),
            Height = 88,
        ...
        panel.Controls.AddRange([name, time, bar]);
        return panel;
    }
    ...
    private Control BuffRow(ActiveTimer timer)
    {
        var panel = new Panel
        {
            Width = TimerCardWidth(_buffPanel),
            Height = 43,
    ...
    {
        foreach (Control parent in
                 _hotPanel.Controls.Cast<Control>()
    ...
    private void SaveAll()
    {
        ...
        _store.SaveSettings(
            new AppSettings
            {
                ...
                LearningMode = _learningMode.Checked,
                WindowWidth = Width,
                WindowHeight = Height
            });
